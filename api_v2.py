...
async def search_product(file: UploadFile = File(...)):
    ...
    try:
        ...
        if exact_match:
            ...
            print(f"  💰 Price: {matched_meta.get('price', 'N/A')}")
        ...
        # Get category for top match
        match_category = "watches"  # default
        for cat, keywords in PRODUCT_CATEGORIES.items():
            if any(kw.lower() in top_match['product_name'].lower() for kw in keywords):
                match_category = cat
                break
        
        # Prepare top 5 results with categories
        top_5_enhanced = []
        ...
        return JSONResponse(
            ...
            content={
                ...
                "message": f"Similar product found: {top_match['product_name']} ({confidence.upper()} match - {similarity_score*100:.1f}%)",
